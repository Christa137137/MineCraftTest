...
class TestExecution:
    ...
    def sleep(self, total_duration):
        ...
        while self.time < target_time:
            step = 1.0 if not self.tracking_registry else 0.5
            ...
            if self.tracking_registry:
                for tag, registry_info in list(self.tracking_registry.items()):
                    ...
                    interval = m_int if obj_type == "mob" else b_int
                    if round(self.time % interval, 2) == 0:
    ...
    # def _get_block_desc(self, x, y, z):
    #     block_info = self.get_block_id(x, y, z).lower()
    #     if "water" in block_info:
    #         level = self.get_block_water_level(x, y, z)
    #         return f"water{level}"
    #     elif "bubble_column" in block_info:
    #         return "bubble_column"
    #     elif "air" in block_info:
    #         return "air"
    #     elif "air" not in block_info and "in range" not in block_info and "-1" not in block_info:
    #         return block_info.replace("minecraft:", "").strip()
    #     return "unknown"
