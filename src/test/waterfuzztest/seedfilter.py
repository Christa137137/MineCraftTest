...
class SeedFilter:
    def __init__(self, input_path=None, output_path=None, is_init=False):
        ...
        # generate 15 reference directions for 5 objectives
        self.ref_dirs = get_reference_directions("das-dennis", 5, n_partitions=2)
        ...
        for f_name in files:
            ...
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    ...
                    # use safe_float
                    obj = np.array([
                        safe_float(fit.get("interaction_complexity", {}).get("fitness", 0)),
                        safe_float(fit.get("fluid_fitness", {}).get("fitness", 0)),
                        safe_float(fit.get("object_fitness", {}).get("fitness", 0)),
                        safe_float(fit.get("environment_fitness", {}).get("fitness", 0)),
                        safe_float(fit.get("buoyancy_inconsistency", {}).get("fitness", 0))
                    ])
    ...
    def filter(self):
        ...
        for i, file_name in enumerate(json_files):
            ...
            try:
                ...
                # 在生成文件名时，也做一次安全的强转
                raw_score = data.get("fitness", {}).get("buoyancy_inconsistency", {}).get("fitness", 0)
                try:
